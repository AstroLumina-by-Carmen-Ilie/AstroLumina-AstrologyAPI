...
# Express API pentru date astrologice
# =============================================================================
...
# Fișiere de configurare runtime (ex: setări API, env-specific config)
COPY --from=builder /app/config ./config
# Middleware Express (auth, rate limiting, error handling, etc.)
COPY --from=builder /app/middleware ./middleware
# Route handlers pentru endpointurile API
COPY --from=builder /app/routes ./routes
# Fișiere de traduceri/localizare pentru răspunsurile API
...
# Constante globale ale aplicației (ex: coduri erori, enum-uri, config valori)
...
# Portul pe care ascultă acest API (diferit de PaymentAPI care e pe 3032)
ENV PORT=3031
...
# Documentează că containerul ascultă pe 3031.
# Nu deschide efectiv portul - asta o face Render sau docker run -p 3031:3031
EXPOSE 3031
